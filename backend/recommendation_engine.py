...
class RecomendaJogosEngine:
    def __init__(self, db_filename='sistema.db'):
        # Pega o diretório do script atual (backend/)
        self.base_dir = os.path.dirname(os.path.abspath(__file__))
        self.db_path = os.path.join(self.base_dir, '..', 'database', db_filename)
        self._init_db()

    def _init_db(self):
        """Inicializa o banco de dados executando o schema caso não exista."""
    ...
    def _verificar_limite_requisicoes(self, usuario_id, conn):
        """RN4 - Limite de Requisições: Máximo de 10 por hora."""
        uma_hora_atras = datetime.datetime.now() - datetime.timedelta(hours=1)
        query = """
            SELECT COUNT(*) FROM log_requisicoes_ia 
            WHERE usuario_id = ? AND data_hora >= ?
        """
        cursor = conn.cursor()
        cursor.execute(query, (usuario_id, uma_hora_atras))
    ...
    def _carregar_dados(self, conn):
        ...
        # Combinar gênero e tags para o vetor de características
    ...
    def gerar_recomendacoes(self, usuario_id, limite=5, genero_filtro=None):
        ...
        try:
            ...
            # Calcular Similaridade de Cosseno (Filtragem por Conteúdo)
            ...
            # Variáveis para Regras de Negócio
            ...
            # Avaliando RN1 (Multiplayer Rápido / Engajamento)
            multi_jogados = jogos_jogados_df[jogos_jogados_df['multiplayer_rapido'] == 1]
            if not multi_jogados.empty:
                media_multi = sum([notas_usuario[j_id] for j_id in multi_jogados['id']]) / len(multi_jogados)
                if media_multi >= 4.0:
                    gosta_multi_rapido = True
                    
            # Avaliando RN2 (Dificuldade Elevada)
            dif_jogados = jogos_jogados_df[jogos_jogados_df['dificuldade_elevada'] == 1]
            if not dif_jogados.empty:
                media_dif = sum([notas_usuario[j_id] for j_id in dif_jogados['id']]) / len(dif_jogados)
                if media_dif >= 4.0:
                    gosta_dif_elevada = True
                    
            # Avaliando RN3 (Narrativa / Obras Adaptadas)
            ...
            if not narr_jogados.empty:
                media_narr = sum([notas_usuario[j_id] for j_id in narr_jogados['id']]) / len(narr_jogados)
                if media_narr >= 4.0:
                    gosta_narrativa = True

            # Calculando escore para jogos NÃO jogados
            recomendacoes = []
            ...
            for idx, row in jogos_df.iterrows():
                ...
                # Escore base: média ponderada das similaridades com jogos bem avaliados
                escore_base = 0
                peso_total = 0
                for j_avaliado in jogos_avaliados:
                    idx_avaliado = jogos_df[jogos_df['id'] == j_avaliado].index[0]
                    sim = cosine_sim[idx_avaliado][idx]
                    nota = notas_usuario[j_avaliado]
                    # Só consideramos similaridades positivas e notas acima da média para puxar para cima
                    if nota >= 3.0: 
    ...
    def _fallback_recomendacao_popular(self, jogos_df, limite, genero_filtro):
        """Recomendação genérica caso usuário não tenha histórico."""
        if genero_filtro:
            jogos_df = jogos_df[jogos_df['genero'] == genero_filtro]
        # Pega os primeiros (em um sistema real seria por média geral de avaliações)
        return jogos_df.head(limite).to_dict('records')

# Teste Simples (Pode ser removido depois)
